...
public class ConsultaDao {
	...
	public Paciente BuscarDadosPaciente(String cpfPaciente) {
		...
		try {
			String sql = "select p.nome, p.cpf, p.sexo, t.numerotelefone " 
					+ "from paciente p "
	...
	public Hospital BuscarDadosHospital(String cnpj) {
		...
		try {
			String sql = "select h.nome, h.cnpj, t.numerotelefone, h.rua, h.bairro, h.cidade " 
					+ "from hospital h "
	...
	public Medico BuscarDadosMedico(String cpf) {
		...
		try {
			String sql = "select m.nome, m.cpf, m.crm, m.especialidade " 
					+ "from medico m " 
					+ "where m.cpf = ?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
